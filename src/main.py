...
from models import db, User, Planet, People, Vehicle, Favorites
...
def get_all_vehicles():
    list_vehicles = Vehicle.get_vehicles()
    
    return jsonify(list_vehicles), 200

@app.route('/vehicles/<vehicles_id>', methods=['GET'])
def get_a_vehicle(vehicles_id):
    vehicles = Vehicle.query.get(vehicles_id)
...
def add_favorite_people(people_id):
    ...
    db.session.add(new_favorite)
    db.session.commit()
    return jsonify("Favorite person added"), 201
...
def delete_favorite_people(people_id):
    ...
    if favorite is not None:
        db.session.delete(favorite)
        db.session.commit()
        return jsonify("Favorite person deleted"), 200


@app.route('/favorite/vehicle/<vehicle_id>', methods=['POST'])
def add_favorite_vehicle(vehicle_id):
    body = request.get_json()
    new_favorite = Favorites(user_id=body["user_id"], vehicle_id=vehicle_id)
    db.session.add(new_favorite)
    db.session.commit()
    return jsonify("Favorite vehicle added"), 201

@app.route('/favorite/vehicle/<vehicle_id>', methods=['DELETE'])
def delete_favorite_vehicle(planet_id):
    body = request.get_json()
    favorite = Favorites.query.filter(Favorites.user_id==body['user_id'], Favorites.vehicle_id==vehicle_id).first()
    if favorite is not None:
        db.session.delete(favorite)
        db.session.commit()
